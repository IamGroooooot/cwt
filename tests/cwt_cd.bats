...
@test "cwt cd: --default-permissions alone does not launch assistant" {
	zsh -c "
    export NO_COLOR=1
    cd '$REPO_DIR'
    source '$CWT_SH'
    cwt new --no-launch cd-default-perm HEAD
  " 2>/dev/null

	run zsh -c "
    export NO_COLOR=1
    export CWT_CMD_CLAUDE='echo SHOULD_NOT_LAUNCH'
    cd '$REPO_DIR'
    source '$CWT_SH'
    cwt cd cd-default-perm --default-permissions
  "
	[ "$status" -eq 0 ]
	[[ "$output" == *"Entered"* ]]
	[[ "$output" != *"SHOULD_NOT_LAUNCH"* ]]
}
